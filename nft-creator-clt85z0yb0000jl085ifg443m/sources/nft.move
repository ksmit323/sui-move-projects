...
module overmind::NonFungibleToken {
    ...
    const MINT_PRICE: u64 = 1000000000; // 1 SUI
    ...
    */
    public fun mint_nft(
        ...
        // Checks
        assert!(coin::value(payment_coin) >= MINT_PRICE, EInsufficientPayment);
        ...
        let paid = balance::split(coin_balance, MINT_PRICE); // split() returns the subtracted value as Balance<T>; in this case the MINT_PRICE
    ...
    */
    public fun combine_nfts(
        ...
        string::append(&mut new_description, nft1.name);
        string::append(&mut new_description, string::utf8(b" and "));
        string::append(&mut new_description, nft2.name);
        ...
        event::emit(NonFungibleTokenCombined{
            nft1_id: object::id(&nft1),
            nft2_id: object::id(&nft2),
            new_nft_id: object::id(&new_nft)
        });

        // Delete combined NFT's
        let NonFungibleToken { id, name: _, description: _, image: _ } = nft1;
        object::delete(id); 
        let NonFungibleToken { id, name: _, description: _, image: _ } = nft2;
        object::delete(id); 

        // Return new NFT object
        new_nft
    ...
    */
    public fun withdraw_sales(
        ...
        let withdrawal_amount = coin::take(&mut minter_cap.sales, amount, ctx);

        // Emit event
        event::emit(SalesWithdrawn{
            amount: coin::value(&withdrawal_amount)
        });

        // Return withdrawn coin
        withdrawal_amount
    }
