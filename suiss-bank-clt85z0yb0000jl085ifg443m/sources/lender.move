...
module overmind::lending {
    ...
    */
    public fun create_pool<CoinType>(
        ...
        // increment number of pools in ProtocolState
    ...
    */
    public fun deposit<CoinType>(
        ...
        let userData = table::borrow_mut(&mut state.users, tx_context::sender(ctx));

        // Increase user's collateral amount in the pool
        let userCollateral = table::borrow_mut(&mut userData.collateral_amount, pool.asset_number);
        *userCollateral = *userCollateral + coin::value(&coin_to_deposit);

        // Add the coin to the pool's reserve
        balance::join(&mut pool.reserve, coin::into_balance(coin_to_deposit));
    }
    ...
    */
    public fun withdraw<CoinType>(
        ...
        // Decrease user's collateral amount in the pool
        let userCollateral = table::borrow_mut(&mut userData.collateral_amount, pool.asset_number);
        *userCollateral = *userCollateral + amount_to_withdraw;

        // Removes coin from the pool's reserve
        let withdraw_from_reserve = balance::split(&mut pool.reserve, amount_to_withdraw);

        // Return coins withdrawn
        coin::from_balance(withdraw_from_reserve, ctx) 
    }
    ...
    */
    public fun borrow<CoinType>(
        ...
        // Increase user's borrowed amount
        let user_borrowed_amount = table::borrow_mut(&mut userData.borrowed_amount, pool.asset_number);
        *user_borrowed_amount = *user_borrowed_amount + amount_to_borrow;

        // Remove coin from the pool's reserve
        let withdraw_from_reserve = balance::split(&mut pool.reserve, amount_to_borrow);

        // Return coin from pool's reserve
        coin::from_balance(withdraw_from_reserve, ctx)
    }
    ...
    */
    public fun repay<CoinType>(
        ...
        // Decrease user's borrowed amount
        let user_borrowed_amount = table::borrow_mut(&mut userData.borrowed_amount, pool.asset_number);
        *user_borrowed_amount = *user_borrowed_amount - coin::value(&coin_to_repay);

        // Add coin to the pool's reserve
        balance::join(&mut pool.reserve, coin::into_balance(coin_to_repay));
    }
    ...
    */
    public fun calculate_health_factor(
        ...
        //! Make sure to multiply by 100 for 2 decimal precision
    ...
    //     ctx: &mut TxContext
    // ): UserData {
    //     table::borrow(state.users, tx_context::sender(ctx))
    // }
