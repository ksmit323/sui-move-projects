...
module overmind::liquidity_pool {
    ...
    */
    public fun create_liquidity_pool<CoinA, CoinB>(
        ...
        let lp_calc = math::sqrt(coin::value(&coin_a) * coin::value(&coin_b));
        assert!(lp_calc >= MIN_LIQUIDITY, EInsufficientLiquidity);

        // Initialize LP supply and distribute initial and provider's LP coins.
        let lp_supply = balance::create_supply(LPCoin<CoinA, CoinB> {});
        let lp_reserve = balance::increase_supply(&mut lp_supply, MIN_LIQUIDITY);
        let lp_coins = balance::increase_supply(&mut lp_supply, lp_calc - MIN_LIQUIDITY);
        ...
        transfer::share_object(LiquidityPool {
            ...
            initial_lp_coin_reserve: lp_reserve
        });

        // Return LP coins to liquidity provider
    ...
    */
    public fun supply_liquidity<CoinA, CoinB>(
        ...
        assert!(coin::value(&coin_a) > 0 && coin::value(&coin_b) > 0, EInsufficientLiquidity);

        let amount_coin_a = coin::into_balance(coin_a);
        let amount_coin_b = coin::into_balance(coin_b); 

        let (
            amount_coin_a_reserve, 
            amount_coin_b_reserve, 
            lp_supply
            ) = get_pool_stuff(pool);

        let lp_coins = math::min(
            balance::value(&amount_coin_a) * lp_supply / amount_coin_a_reserve, 
            balance::value(&amount_coin_b) * lp_supply / amount_coin_b_reserve
        );
     
        // Add to pool balances
        balance::join(&mut pool.coin_a_balance, amount_coin_a);
        balance::join(&mut pool.coin_b_balance, amount_coin_b);

        let mint_lp_coins = balance::increase_supply(&mut pool.lp_coin_supply, lp_coins);
        coin::from_balance(mint_lp_coins, ctx)
    }
    ...
    */
    public fun remove_liquidity<CoinA, CoinB>(
        ...
        let amount_lp_coins = balance::decrease_supply(&mut pool.lp_coin_supply, coin::into_balance(lp_coins_to_redeem));

        assert!(amount_lp_coins > 0, EInsufficientLiquidity);

        // Calculate coin amounts to return
        let amount_coin_a = amount_lp_coins * amount_coin_a_reserve / lp_supply;
        let amount_coin_b = amount_lp_coins * amount_coin_b_reserve / lp_supply;

        (
            coin::take(&mut pool.coin_a_balance, amount_coin_a, ctx),
            coin::take(&mut pool.coin_b_balance, amount_coin_b, ctx)
        )
    }
    ...
    */
    public fun swap_exact_a_for_b<CoinA, CoinB>(
        ...
        assert!(amount_coin_a_in > 0 && min_amount_coin_b_out > 0, EInvalidSwapParameters);
        ...
        let amount_coin_b_out = get_amount_coin_out(
            amount_coin_a_in,
            amount_coin_a_reserve, 
            amount_coin_b_reserve
        );

        assert!(amount_coin_b_out >= min_amount_coin_b_out, ESlippageLimitExceeded);

        balance::join(&mut pool.coin_a_balance, coin::into_balance(coin_a_in));
        coin::take(&mut pool.coin_b_balance, amount_coin_b_out, ctx) // function returns amount_coin_b_out as Coin<CoinB>
    }
    ...
    */
    public fun swap_exact_b_for_a<CoinA, CoinB>(
        ...
        assert!(amount_coin_b_in > 0 && min_amount_coin_a_out > 0, EInvalidSwapParameters);
        ...
        let amount_coin_a_out = get_amount_coin_out(
            amount_coin_b_in,
            amount_coin_b_reserve,
            amount_coin_a_reserve
        );

        assert!(amount_coin_a_out >= min_amount_coin_a_out, ESlippageLimitExceeded);

        balance::join(&mut pool.coin_b_balance, coin::into_balance(coin_b_in));
        coin::take(&mut pool.coin_a_balance, amount_coin_a_out, ctx)
    }
    ...
    */
    public fun swap_a_for_exact_b<CoinA, CoinB>(
        ...
        let max_amount_coin_a_in = coin::value(max_coin_a_in);
        assert!(max_amount_coin_a_in > 0 && amount_coin_b_out > 0, EInvalidSwapParameters);
        ...
        let amount_coin_a_in = get_amount_coin_in(
            amount_coin_b_out,
            amount_coin_a_reserve,
            amount_coin_b_reserve
        );

        assert!(amount_coin_a_in <= max_amount_coin_a_in, ESlippageLimitExceeded);

        let coin_a_in = coin::split(max_coin_a_in, amount_coin_a_in, ctx);

        balance::join(&mut pool.coin_a_balance, coin::into_balance(coin_a_in));
        coin::take(&mut pool.coin_b_balance, amount_coin_b_out, ctx)
    }   
    ...
    */
    public fun swap_b_for_exact_a<CoinA, CoinB>(
        ...
        let max_amount_coin_b_in = coin::value(max_coin_b_in);
        assert!(max_amount_coin_b_in > 0 && amount_coin_a_out > 0, EInvalidSwapParameters);
        ...
        let amount_coin_b_in = get_amount_coin_in(
            amount_coin_a_out,
            amount_coin_b_reserve,
            amount_coin_a_reserve
        );

        assert!(amount_coin_b_in <= max_amount_coin_b_in, ESlippageLimitExceeded);
    ...
    public fun get_pool_stuff<CoinA, CoinB>(
        pool: &LiquidityPool<CoinA, CoinB>
        ): (u64, u64, u64) {
        (
            balance::value(&pool.coin_a_balance),       // Coin A reserves
            balance::value(&pool.coin_b_balance),       // Coin B reserves
            balance::supply_value(&pool.lp_coin_supply) // LP supply
        )
    }

    public fun get_amount_coin_out(
        amount_in: u64, prev_in_reserve: u64, prev_out_reserve: u64
        ): u64 {

        // Per spec, cast to u128 to prevent overflow
        let (
            amount_in, 
            prev_in_reserve, 
            prev_out_reserve
                ...
                (amount_in as u128), 
                (prev_in_reserve as u128), 
                (prev_out_reserve as u128)
            );

        let new_in_reserve = prev_in_reserve + amount_in;
        let new_out_reserve = prev_in_reserve * prev_out_reserve / new_in_reserve;
        let amount_out = prev_out_reserve - new_out_reserve;

        (amount_out as u64)
    }

    public fun get_amount_coin_in(
        amount_out: u64, prev_in_reserve: u64, prev_out_reserve: u64
    ): u64 {
        ...
        let new_out_reserve = prev_out_reserve - amount_out;
        let new_in_reserve = prev_in_reserve * prev_out_reserve / new_out_reserve;
        let amount_in = new_in_reserve - prev_in_reserve;
